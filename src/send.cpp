...
static int progress_callback(void *clientp, curl_off_t total_to_download,
	...
	double progress =
	    static_cast<double>(now_upload) / total_to_upload * 100.0;
...
void sendfiles(nlohmann::json &client, nlohmann::json &fileinfos) {
	...
	if (curl) {
		...
		auto baseurl = schema + "://" + ip + ":" +
			       std::to_string(PORT) +
			       "/api/localsend/v2/"
			       "upload?";
		baseurl.append("sessionId=")
		    .append(
			fileinfos["upload"]["sessionId"].get<std::string>());
		for (auto &f : fileinfos["upload"]["files"].items()) {
			...
			auto realpath =
			    fileinfos["realpath"][f.key()].get<std::string>();
			...
			std::cout << "开始传输: "
				  << fileinfos["files"][f.key()]["fileName"]
					 .get<std::string>()
				  << std::endl;
			res = curl_easy_perform(curl);

			if (res != CURLE_OK) {
				std::cerr << "curl_easy_perform() 失败: "
...
void send(const std::vector<std::string> &files) {
	nlohmann::json fileinfos;
	if (!checkfiles(files, fileinfos)) exit(1);
	run.store(true);
	std::thread listener(start_listener);
	std::string input;

	std::cout << "开始搜索客户端 ..." << std::endl;
	std::cout << highlight << "按 Enter 键停止搜索" << highlight_end
		  << std::endl;
	...
	while (true) {
		std::cout << "请输入编号或回车选择" << highlight << " 1(1 - "
			  << clients.size() << "): " << highlight_end;
		...
		if (index > 0 && index <= clients.size()) break;
		std::cout << "无效输入" << std::endl;
	}
	...
	std::map<long, std::string> prepareerrors{{204, "完成(无需传输文件)"},
						  {400, "无效请求体"},
						  {403, "拒绝"},
						  {500, "接收器未知错误"}};
	if (response.contains("error")) {
		auto code = response["error"].get<long>();
		if (prepareerrors.count(code))
			std::cout << prepareerrors[code] << std::endl;
		else
			std::cout << "未知返回代码" << std::endl;
		return;
	}
